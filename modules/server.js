...
var MIME_TYPES = require('./mime_types');
var template_directory = FS.readFileSync('./modules/template-listing.js');
...
function handleRequest( url, callback ){
	...
	if ( PATH.extname( url ) === '' ){
		var path = CONFIG.site_base + '/index.html';
		FS.exists( path, function ( path_exists ){
			if ( path_exists ){
				getFileResponse( url + '/index.html', function ( response_object ){
...
//						Custard template
						...
						template.addTagSet( 'h', require('./modules/custard/templates/tags/html') );
